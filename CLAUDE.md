...
- **CLI layer** (`src/commands/`): Thin wrappers — parse args → call client → format output → catch errors. Nothing imports from `commands/`; it only imports from `src/core/`. Organized noun-verb: each noun (task, project, tag, folder, inbox, bulk) is a directory with an `index.ts` registering verb files. Standalone commands (forecast, review, stats, collect, completion) attach to the root program. **Root shortcuts** (`src/commands/shortcuts.ts`) mount selected task verbs on the root as well — `of complete` *is* `registerCompleteCommand` registered twice, never a second implementation; `of move` is mounted the same way; add future shortcuts (`edit`, ...) to its `TASK_SHORTCUTS` list. New verbs should wrap their handler in `runAction()` (`src/commands/action.ts`), which resolves the output format and turns any thrown `CLIError`/`BridgeError` into `outputError` + `process.exit(exitCode)` — so a verb body just throws. Shell completions are generated from the live Commander tree (`generateCompletionScript`), not hardcoded — a parity test enforces coverage.
