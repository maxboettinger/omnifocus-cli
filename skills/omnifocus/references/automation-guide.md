...
### add_subtask.js
Add a child task (sub-task) under any existing OmniFocus task. Supports arbitrary nesting depth for the full taxonomy hierarchy: 📦 Package → 🗂️ Task → 👣 Step.

**Usage:**
```bash
osascript -l JavaScript add_subtask.js "Subtask name" --parent "Parent name or ID" [options]
osascript -l JavaScript add_subtask.js "Subtask name" --parent-id "OmniFocus ID" [options]
```

**Options:** `--note`, `--due`, `--defer`, `--tag` (repeatable), `--flag`, `--estimate` (same as add_inbox.js)

**Parent resolution:** ID → exact name match → substring match. If ambiguous, returns candidates with IDs.

**Examples:**
```bash
# By parent name
osascript -l JavaScript add_subtask.js "☑️🪫 Belege sortieren" --parent "🗂️ Steuererklärung" --estimate 15

# By parent ID (most reliable)
osascript -l JavaScript add_subtask.js "☑️🪫 PDF hochladen" --parent-id "abc123" --estimate 5

# Deep nesting (sub-sub-task)
osascript -l JavaScript add_subtask.js "☑️🪫 Seite 3 prüfen" --parent "Belege sortieren"
```

**Output:** JSON `{ ok, id, name, note, dueDate, deferDate, tags, flagged, estimateMin, parent: { id, name, project } }`
