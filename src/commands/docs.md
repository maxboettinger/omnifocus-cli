...
- **Error handling**: Verb actions wrap their body in try/catch and pass the caught error object straight to `outputError()` (not a pre-formatted string) — this lets structured data, like a `BridgeError`'s disambiguation `candidates`, survive into JSON-mode stderr output. See `@/src/core/docs.md` for `outputError`'s TTY-dependent rendering.
