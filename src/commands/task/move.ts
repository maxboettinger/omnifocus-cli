...
export function registerMoveCommand(parent: Command, client: OmniFocusClient): void {
	parent
		...
		.argument("[ref]", "Task reference: short id or name query (can be omitted with --id)")
		.argument("[due]", "New due date, e.g. tomorrow, 'fri 5pm', 2d, 2026-09-10, clear")
		.option("--id <id>", "Task ID")
		.option("--defer <date>", "New defer date (or 'clear')")
		.option("--planned <date>", "New planned date (or 'clear')")
