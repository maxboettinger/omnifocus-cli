...
- **Output dual-mode**: `resolveFormat()` returns `"json"` if `--json` is passed or stdout is not a TTY, otherwise `"human"`. Every entity type (task, project, tag, folder) has both a line formatter (for lists) and a detail formatter, plus a list outputter that handles the empty-state message and count footer. Color output (`colorEnabled(stream)` / `paint()`) is gated per-stream: `NO_COLOR` (any non-empty value) forces it off, `FORCE_COLOR` (set and not `"0"`) forces it on, otherwise it follows that specific stream's own `isTTY` — stdout and stderr are checked independently, so a colored terminal session piping only stderr to a file still gets stderr's own TTY-detection.
...
- **`readStdin(example)`** (`@/src/core/stdin.ts`) is the shared stdin reader for the bulk create/update/complete commands and `inbox process-many`. It throws a `CLIError` immediately if `process.stdin.isTTY` is true, showing a usage example in the message — otherwise a command run without piped input would hang forever waiting for EOF. All four call sites import this one implementation rather than each defining its own reader.
